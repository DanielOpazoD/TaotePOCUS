...
for (const c of slice) {
  ...
  imported.push({
    ...
    // Audit metadata: which tweet this came from. Stripped before
    // serialization. We don't carry it into the JSON because the
    // app's `CaseRecord` type doesn't declare it.
    __twitter: {
...
// ─── Emit public/data/imported-cases.json ───────────────────────────────────
//
// Pre-Bloque-O this script wrote a TypeScript array literal to
// `lib/imported-cases.ts` and the loader did `import("./imported-cases")`
// (code-split chunk). The corpus is now a static JSON asset under
// `public/`, fetched on demand and cached by the CDN. See
// `lib/seed-cases.ts` for the consumer side.

const outPath = join(PROJECT_ROOT, "public", "data", "imported-cases.json");

// Strip the audit metadata before serializing — the JSON consumer
// doesn't declare it. The provenance trail (source URL, engagement)
// is preserved separately in `imported-cases.audit.json` for any
// future "where did this case come from?" tooling.
const dataPayload = imported.map((c) => {
  // Strip the audit metadata before serializing — the public JSON
  // doesn't carry it. The provenance trail lives in the sidecar
  // emitted just below.
...
writeFileSync(outPath, JSON.stringify(dataPayload, null, 2) + "\n");

// Audit sidecar — same length and order as the data payload. Useful
// for debugging "case X came from which tweet?" without having to
// search Twitter. Not loaded by the app at runtime.
const auditPath = join(PROJECT_ROOT, "public", "data", "imported-cases.audit.json");
const auditPayload = imported.map((c) => ({
  id: c.id,
  sourceUrl: c.__twitter.sourceUrl,
  engagement: c.__twitter.engagement,
  classifiedCategory: c.__twitter.classifiedCategory,
  generatedAt: new Date().toISOString(),
  archivePath,
}));
writeFileSync(auditPath, JSON.stringify(auditPayload, null, 2) + "\n");
...
console.log(`Audit sidecar:    ${auditPath}`);
console.log("");
console.log("Next steps:");
console.log("  1. Run: npm run typecheck && npm test");
console.log("  2. Open localhost:3010 and review the imported cases.");
console.log("     The corpus is loaded lazily via fetch from /data/imported-cases.json.");
