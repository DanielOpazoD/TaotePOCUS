...
export interface CaseRecord {
  ...
  purged?: boolean;
  /**
   * Provenance metadata for AI-assisted translations. Optional —
   * cases authored by hand never carry this field. Set by the
   * `/api/admin/ai/translate` route handler when the admin accepts
   * an AI suggestion into the EN slot (or vice versa). The
   * `tests/ai-translation-flag-audit.test.ts` static check enforces
   * the invariant that anything `aiGenerated: true` either has
   * `reviewedAt` set within 30 days OR is still in admin draft
   * state. Public surfaces NEVER read this — it's audit data, not
   * editorial signal.
   */
...
/**
 * Provenance metadata for an AI-assisted translation slot. Attached
 * to `CaseRecord.translationMeta` when an admin accepts an AI
 * suggestion into the EN slot (or vice versa for an EN-source case).
 *
 * Audit invariants enforced by `tests/ai-translation-flag-audit.test.ts`:
 *   - `aiGenerated: true` ⇒ `reviewedAt` must be set within 30 days
 *     OR the case is a draft (not published). This prevents stale
 *     unreviewed AI content from accumulating in production.
 *   - `provider` MUST match a known id in `lib/ai/registry.ts`.
 *
 * Never read by public surfaces. Used by:
 *   - The admin "AI Suggestions" panel to surface "generated by X
 *     model on Y date, not yet reviewed" badges.
 *   - The future quality-monitoring dashboard.
 *   - The static audit.
 */
export interface TranslationMeta {
  /**
   * `true` when the EN (or ES) slot was populated by an AI provider
   * rather than written by hand. Default state for AI-generated
   * content; once an admin edits the content meaningfully (>20% of
   * tokens change) this flips back to `false`.
   */
  aiGenerated: boolean;
  /** Which provider id produced the translation. */
  provider: "stub" | "gemini" | "openai" | "deepseek";
  /** Specific model used (e.g., "gemini-2.5-flash"). */
  model: string;
  /** ISO timestamp when the AI generated the suggestion. */
  generatedAt: string;
  /**
   * ISO timestamp when an admin reviewed + accepted the suggestion.
   * Absent until human review happens. The audit gate flags
   * `aiGenerated: true && reviewedAt == null` cases that linger
   * past 30 days.
   */
  reviewedAt?: string;
